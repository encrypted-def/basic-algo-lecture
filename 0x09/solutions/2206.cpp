...
// Co-authored by : -
// http://boj.kr/9c8a4f4d1c9146dc93118490764dd391
#include <bits/stdc++.h>
using namespace std;
#define X first
#define Y second
...
int main(void) {
  ...
  cin >> n >> m;
  boardEnd = { m - 1, n - 1 };
  for (int i = 0; i < n; ++i) for(int j = 0; j < m; ++j) cin >> board[j][i];
  for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j) cost[j][i][0] = cost[j][i][1] = -1;

...
// Co-authored by : -
// http://boj.kr/803c8b6d06fc4a118632211e83d851d5
#include <bits/stdc++.h>
using namespace std;
...
각도를 입력 받아 정렬한 뒤 가장 작은 각도에 360,000을 더한 값을 n번째 인덱스로 추가한다.
이후 앞뒤 각도의 차이를 배열 a, b에 저장한다.

a에 대한 실패함수를 구한다. 이후 a와 b를 매칭한다.
원형 패턴이므로, 시작 인덱스가 0일 때부터 (n - 1)일 때까지 매칭을 시도한다.

시작 인덱스가 i이어서 증가하던 중 (n - 1)보다 커져 n이 된 경우,
다시 0으로 돌아가도록 구현한다(17번째 줄).
이렇게 실제 b배열 상에서 가리키는 인덱스를 ci라는 변수로 활용한다.

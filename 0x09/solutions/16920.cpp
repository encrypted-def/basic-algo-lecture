...
// Co-authored by : -
// http://boj.kr/7ec87f5e05d04ac785abcf49bed8bab9
#include <bits/stdc++.h>
using namespace std;
...
int main() {
  ...
  // 큐 배열에 큐 생성
  for(int i=1; i<=P; i++)
    q[i] = queue<pair<pair<int ,int>, int>>();
  // 각 플레이어의 이동 제한 횟수 입력
  for(int i=1; i<=P; i++)
    cin >> step[i];
  // 맵 입력
  ...
  while(1){
    ...
    for (int i = 1; i <= P; i++) {
      queue<pair<pair<int, int>, int>> nextq; // 다음 턴에 확장을 고려할 영토 기록
      while (!q[i].empty()) {
        int curx = q[i].front().first.first, cury = q[i].front().first.second;
        int curstep = q[i].front().second;
        q[i].pop();
        if (curstep >= step[i]) {
